...
const TagTemplate: React.FC<PageProps<TagTemplateData, TagTemplateContext>> = ({ pageContext, data, location }) => {
  ...
  const edges = allEdges.filter(({ node }) => {
    ...
    return tags.some((t: string) => (t || '').toLowerCase() === displayTag.toLowerCase());
  });
